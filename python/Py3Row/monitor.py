...
Distributed for free, and for non commercial use.
Copyright Domenico De Vivo, please do not copy.
...
# load all images - assumed to be relative to script
...
def main():
    ...
    while start_game is False:
        try:
            ...
            if monitor["time"] > 0:  # if rowing detected start main loop
                start_game = True
        except Exception:  # display error msg
    ...
    while game_over is False:
        ...
        if view == 0:
            ...
            timeText = displayFont.render(
                "%02d:%02d " % (wtime_mins, wtime_secs), True, BLUE
            )
            tenthsText = display_smFont.render("." + wtime_tenths, True, BLUE)
            paceText = displayFont.render(
                "%02d:%02d " % (pace_mins, pace_secs), True, GOLD
            )
        ...
        if view == 2:
            ...
            for f in force:
                ...
                (y_axis height) to plot the curve starting at bottom since
