...
class TestRomanNumerals:
    ...
    def test_small_number(self):
        res = convert.romanToInt(self.small_num)
        assert 4 == res

    def test_big_number(self):
        res = convert.romanToInt(self.big_num)
        assert 1904 == res

    def test_invalid_sequence(self):
        with pytest.raises(Exception):
            res = convert.romanToInt(self.invalid_seq)

    def test_invalid_characters(self):
        with pytest.raises(KeyError):
            res = convert.romanToInt(self.invalid_char)
...
class TestBinary:
    def test_8bit(self):
        ...
        msb = 2 ** (bits - 1)
        m_lsb = msb + 1
        res = convert.toBinStr(1, bits)
        assert "00000001" == res
        res = convert.toBinStr(msb, bits)
        assert "10000000" == res
        res = convert.toBinStr(m_lsb, bits)
        assert "10000001" == res
    ...
    def test_raise_overflow_Exception(self):
        ...
        with pytest.raises(OverflowError):
            res = convert.toBinStr(2 ** bits, bits)

    def test_set_all_bits(self):
        bits = 8
        msb = 2 ** (bits - 1)
        m_lsb = msb + 1
        res = convert.toBinStr(2 ** bits - 1, bits)
        assert "11111111" == res
